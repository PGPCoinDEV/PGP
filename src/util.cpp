...
int nAnonymizePrettyGoodPrivacyAmount = 1000;
...
#else
    const char* pszModule = "prettygoodprivacy";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PrettyGoodPrivacy
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PrettyGoodPrivacy
    // Mac: ~/Library/Application Support/PrettyGoodPrivacy
    // Unix: ~/.PrettyGoodPrivacy
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PrettyGoodPrivacy";
#else
    fs::path pathRet;
    ...
    return pathRet / "PrettyGoodPrivacy";
#else
    // Unix
    return pathRet / ".prettygoodprivacy";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "prettygoodprivacy.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "prettygoodprivacyd.pid"));
