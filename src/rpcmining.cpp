...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "PrettyGoodPrivacy is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "PrettyGoodPrivacy is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "PrettyGoodPrivacy is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "PrettyGoodPrivacy is downloading blocks...");
