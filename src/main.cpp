...
unsigned int nStakeMinAge = 6 * 60 * 60; // 6 hour's
...
int nCoinbaseMaturity = 60; // 80 in total
...
const string strMessageMagic = "PrettyGoodPrivacy Signed Message:\n";
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if (pindexBest-> nHeight < 10000)
    {
        nSubsidy = nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 200; // 10,000% yearly interest
    }
...
static int64_t nTargetTimespan = 16 * 60;  // 16 minutes
...
{
    RenameThread("prettygoodprivacy-loadblk");
