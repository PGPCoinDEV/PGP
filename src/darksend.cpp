    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat PrettyGoodPrivacy and make it unusable. To
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizePrettyGoodPrivacyAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fPrettyGoodPrivacySubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f PGP CENT - sessionTotalValue %d\n", fPrettyGoodPrivacySubmitted, sessionTotalValue);
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("prettygoodprivacy-darksend");
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizePrettyGoodPrivacyAmount = (rand() % nLeftToAnon)+3;
                } else {
