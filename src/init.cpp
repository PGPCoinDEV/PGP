...
void Shutdown()
{
    ...
    RenameThread("prettygoodprivacy-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: prettygoodprivacy.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: prettygoodprivacyd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 16432 or testnet: 26432)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 16433 or testnet: 26433)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeprettygoodprivacyamount=<n> " + _("Keep N PrettyGoodPrivacy anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. PrettyGoodPrivacy is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. PrettyGoodPrivacy is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("PrettyGoodPrivacy version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "PrettyGoodPrivacy server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of PrettyGoodPrivacy") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart PrettyGoodPrivacy to complete") << "\n";
    ...
    nAnonymizePrettyGoodPrivacyAmount = GetArg("-anonymizeprettygoodprivacyamount", 0);
    if(nAnonymizePrettyGoodPrivacyAmount > 999999) nAnonymizePrettyGoodPrivacyAmount = 999999;
    if(nAnonymizePrettyGoodPrivacyAmount < 2) nAnonymizePrettyGoodPrivacyAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizePrettyGoodPrivacyAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize PrettyGoodPrivacy Amount %d\n", nAnonymizePrettyGoodPrivacyAmount);
