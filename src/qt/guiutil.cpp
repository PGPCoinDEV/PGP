...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("prettygoodprivacy"))
        return false;
...
{
    // Convert prettygoodprivacy:// to prettygoodprivacy:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("prettygoodprivacy://"))
    {
        uri.replace(0, 12, "prettygoodprivacy:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "PrettyGoodPrivacy.lnk";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart)
    {
        ...
        // Get a pointer to the IPrettyGoodPrivacyLink interface.
        IShellLink* psl = NULL;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            // Query IPrettyGoodPrivacyLink for the IPersistFile interface for
...
{
    return GetAutostartDir() / "prettygoodprivacy.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        optionFile << "Name=PrettyGoodPrivacy\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("PrettyGoodPrivacy-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  prettygoodprivacy-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("PrettyGoodPrivacy-Qt"));
